...
for some common detection, we improved [includes](https://xmake.io/#/zh/manual?id=includes) interface to provide some built-in packaged auxiliary interface functions to quickly implement the above option detection and write config.h.
