...
通过此接口，可以从前面[add_files](https://xmake.io/#/zh/manual?id=targetadd_files)接口添加的文件列表中，删除指定的文件，例如：
...
例如，我们可以条件判断来控制删除哪些文件，并且此接口也支持[add_files](https://xmake.io/#/zh/manual?id=targetadd_files)的匹配模式，过滤模式，进行批量移除。
...
对于有些文件，需要支持多文件构建生成单一对象的模式，可以通过[on_build_all](https://xmake.io/#/zh/manual?id=ruleon_build_all)来实现：
...
之前的版本，使用[option](https://xmake.io/#/zh/manual?id=option)可实现命令行菜单选项的用户自定义，当工程配置相当多的情况下，采用这种命令行配置的方式就不是很灵活了。

因此在2.1.9版本中，我们扩展了option，使其原生支持`xmake f --menu`的图形化配置界面，实现复杂的分级配置，并且支持配置的模糊查找和定位，配置项目更加灵活方便。

我们可以通过[set_category](https://xmake.io/#/zh/manual?id=optionset_category)设置option的分级路径名`set_category("root/submenu/submenu2")`，例如：
...
并且我们还可以通过[set_values](https://xmake.io/#/zh/manual?id=optionset_values)，提供选项值列表供用户快速选择使用，例如：
