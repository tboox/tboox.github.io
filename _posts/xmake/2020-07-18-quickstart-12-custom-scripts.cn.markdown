...
这些独立的lua脚本里面，我们还可以通过[import](https://xmake.io/zh/manual/builtin_modules?id=import)导入各种内置模块和自定义模块进来使用，就跟平常写lua, java没啥区别。
...
* 自定义脚本([on_build](https://xmake.io/zh/manual/project_target?id=targeton_build), [on_run](https://xmake.io/zh/manual/project_target?id=targeton_run) ..)
* 插件开发
* 模板开发
* 平台扩展
* 自定义任务task
...
而`*.h`和`**.h`中的匹配模式，跟[add_files](https://xmake.io/zh/manual/project_target?id=targetadd_files)中的类似，前者是单级目录匹配，后者是递归多级目录匹配。
...
这块的具体详情和差异，还有更多os接口，都可以到：[os接口文档](https://xmake.io/zh/manual/builtin_modules?id=os) 查看。
...
更多内置模块详情见：[内置模块文档](https://xmake.io/zh/manual/builtin_modules)
